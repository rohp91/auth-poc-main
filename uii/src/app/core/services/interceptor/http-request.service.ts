...
export class HttpRequestService implements HttpInterceptor {
  ...
  intercept(
    ...
    if(appendAuthorization && environment.SUBSCRIPTION_KEY){
      requestHeaders['Ocp-Apim-Subscription-Key'] = environment.SUBSCRIPTION_KEY;
      requestHeaders['Ocp-Apim-Trace'] = 'true';
    }
    if(appendXtraceId){
